...
public class Common implements ModInitializer {
	...
	public void onInitialize() {
		Item grayShark = Registry.register(Registry.ITEM, KLAPPAR_HAJ_ID, new CuddlyItem(new FabricItemSettings().maxCount(1).group(ItemGroup.MISC), "item.blahaj.gray_shark.tooltip"));
		Registry.register(Registry.ITEM, BLAHAJ_ID, new CuddlyItem(new FabricItemSettings().maxCount(1).group(ItemGroup.MISC), "item.blahaj.blue_shark.tooltip"));
		Registry.register(Registry.ITEM, BREAD_ID, new CuddlyItem(new FabricItemSettings().maxCount(1).group(ItemGroup.MISC), null));
		...
		TradeOfferHelper.registerWanderingTraderOffers(1,factory -> {
			factory.add((entity, random) -> new TradeOffer(
					new ItemStack(Items.EMERALD, 15), new ItemStack(grayShark),
					2, 30, 0.1f));
		});
	}
